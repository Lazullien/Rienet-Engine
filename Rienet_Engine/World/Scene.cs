...
namespace Rienet
{
    ...
    {
        public WorldBody world { get; private set; } = new();
        public int ID;
        ...
        public List<Tile[,]> SceneMap { get; private set; } = new();
        /// <summary>
        /// Used for drawing scene, based on virtual tile layers
        /// </summary>
        public List<Tile[,]> VirtualSceneMap { get; private set; } = new();
        public Dictionary<Vector2, HitboxChunk> HitboxChunks { get; private set; } = new();
        public List<PhysicsBody> BodiesInScene { get; private set; } = new();
        public List<Entity> EntitiesInScene { get; private set; } = new();
        public Dictionary<Clickable, ClickableState> SceneClickableStates { get; private set; } = new();
        /// <summary>
        ...
        /// <summary>
        /// An auto area for depositing lights within the scene, could instead manually have lights (like light carrying entities), in which case draw on its own don't add it to this list
        /// </summary>
        ...
        /// <summary>
        /// Bodies requesting update after some time, basically an update queue, 
        ...
        /// <summary>
        /// the layer and scene are added because of the uncertainty the tile would exist to provide the info, but the Layer is there to save time and power running this multiple times.
        /// </summary>
        public bool GetGridInfo(Vector2 pos, int layer, Scene scene, Tile[,] Layer, out TilePlace tile)
        {
            int X = (int)pos.X, Y = (int)pos.Y;
            bool InRange = X >= 0 && X < W && Y >= 0 && Y < H;
            if (InRange) tile = Layer[X, Y] != null ? TilePlace.ToTilePlace(Layer[X, Y]) : new TilePlace(new(X, Y), layer, false, scene); else tile = null;
            return tile != null;
        }
    ...
    public class Background
    {
        ...
        //layer pos in measured in world sizes (1 = 1 tile), pos is measured in screen size (literal pixels on screen, not recommended)
        ...
        public void Draw(Vector2 POV, Camera Cam, SpriteBatch spriteBatch)
        {
            ...
            {
                //count displacement from pov, add to pos (displacementspeed should never be 0, or it just moves with position so perfectly it stays fixed at 0, 0 to the camera)
                ...
                Vector2 Drawpos = GraphicsRenderer.ToScreenPos(DrawposInScene, Cam.pos) + pos;
                spriteBatch.Draw(l.Texture, Drawpos, null, Color.White, 0, Vector2.Zero, Scale, SpriteEffects.None, 1);
            }
        }
    }
