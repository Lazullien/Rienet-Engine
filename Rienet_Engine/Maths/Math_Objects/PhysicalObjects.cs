...
namespace Rienet
{
    ...
    public class AnimatedBody : PhysicsBody
    {
        ...
        public AnimatedBody(IBodyVenue BelongedObject, Vector2 pos, Vector2 size, List<Hitbox> hitbox, float inertia, float Roughness, bool Collidable, Scene BelongedScene, bool Animating) : base(BelongedObject, pos, size, hitbox, inertia, Roughness, Collidable, BelongedScene)
        {
            animator = new(true);
        }
    ...
    public class PhysicsBody : IGameObject
    {
        ...
        {
            if (GamePanel.Time * GamePanel.ElapsedTime == time)
            {
                action(this);
            }
        }
